...
create_endpoint () {
  case $1 in
    ...
    ;;
    orchestration)
    keystone endpoint-create --region $KEYSTONE_REGION --service-id $2 --publicurl 'http://'"$EXT_HOST_IP"':8004/v1/$(tenant_id)s' --adminurl 'http://'"$EXT_HOST_IP"':8004/v1/$(tenant_id)s' --internalurl 'http://'"$EXT_HOST_IP"':8004/v1/$(tenant_id)s'
    ;;
    cloudformation)
    keystone endpoint-create --region $KEYSTONE_REGION --service-id $2 --publicurl 'http://'"$EXT_HOST_IP"':8000/v1' --internalurl 'http://'"$EXT_HOST_IP"':8000/v1' --adminurl 'http://'"$EXT_HOST_IP"':8000/v1'
  esac
}
